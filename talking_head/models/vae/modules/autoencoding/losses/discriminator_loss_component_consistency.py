...
class GeneralLPIPSWithDiscriminatorComponentConsistency(nn.Module):
    def __init__(
        self,
        disc_start: int,
        ...
        self.long_temporal_consistency_loss = AdaptiveTemporalConsistencyLoss(threshold=0.2)
    ...
    def calculate_c_adaptive_weight(
        self, nll_loss: torch.Tensor, g_loss: torch.Tensor, last_layer: torch.Tensor, 
    ) -> torch.Tensor:
        nll_grads = torch.autograd.grad(nll_loss, last_layer, retain_graph=True)[0]
        g_grads = torch.autograd.grad(g_loss, last_layer, retain_graph=True)[0]
        ...
        d_weight = torch.norm(nll_grads) / (torch.norm(g_grads) + 1e-4)
        d_weight = torch.clamp(d_weight, 0.0, 1e4).detach()
        d_weight = d_weight * self.consistency_weight 
        return d_weight
    ...
    def forward(
        ...
        *,  # added because I changed the order here
        ...
        num_frames = inputs.shape[0]/self.batch_size
        if self.consistency_weight > 0 and num_frames > 1:
            if len(inputs.shape) == 4:
                inputs = rearrange(inputs, "(b t) c h w -> b c t h w", b = self.batch_size)
            if len(reconstructions.shape) == 4:
                reconstructions = rearrange(reconstructions, "(b t) c h w -> b c t h w", b = self.batch_size) 
            consistency_loss = self.short_temporal_consistency_loss(inputs.contiguous(), reconstructions.contiguous())
            consistency_loss += self.long_temporal_consistency_loss(inputs.contiguous(), reconstructions.contiguous()) 
            inputs = rearrange(inputs, "b c t h w -> (b t) c h w", b = self.batch_size) 
            reconstructions = rearrange(reconstructions, "b c t h w -> (b t) c h w", b = self.batch_size) 
        else:
        ...
        if optimizer_idx == 0:
            ...
            if self.consistency_weight > 0 and num_frames > 1:
                if self.training:
                    c_weight =  self.calculate_c_adaptive_weight(
                            nll_loss, consistency_loss, last_layer=last_layer
                        )
                else:
                    c_weight = torch.tensor(0.0)
            else:
                c_weight = torch.tensor(0.0)

            loss = weighted_nll_loss + d_weight * self.disc_factor * g_loss +  c_weight * consistency_loss
            log = dict()
            ...
            log.update(
                {
                    ...
                    f"{split}/loss/consistency_loss": consistency_loss.detach().mean(),
    ...
    def short_temporal_consistency_loss(self, input_video: torch.Tensor, reconstruction_video: torch.Tensor):
        """
        Compute the temporal consistency loss between the input video and the reconstructed video.
        
        Args:
            input_video (torch.Tensor): The original input video tensor with shape (b, c, t, h, w).
            reconstruction_video (torch.Tensor): The reconstructed video tensor with the same shape as input_video.
        
        Returns:
            torch.Tensor: The computed temporal consistency loss.
        """
        # Calculate the difference between consecutive frames for both videos
        input_diff = input_video[:, :, 1:, :, :] - input_video[:, :, :-1, :, :]
        reconstruction_diff = reconstruction_video[:, :, 1:, :, :] - reconstruction_video[:, :, :-1, :, :]
        
        # Calculate the L1 loss between the frame differences of the two videos
        consistency_loss = torch.mean(torch.abs(input_diff - reconstruction_diff))
        
        return consistency_loss
    

class AdaptiveTemporalConsistencyLoss:
    def __init__(self, threshold=0.2, max_step=5):
        """
        Compute the adapative long-term temporal consistency loss between the input video and the reconstructed video.
        
        Args:
            threshold (float): used for adjust long-term steps
        """
    ...
    def compute_frame_similarity(self, frame1, frame2):
        ...
        similarity = torch.mean(torch.abs(frame1 - frame2))
        return similarity

    def compute_dynamic_step(self, input_video, start_frame):
        """
        Adjust step based on content similarity
        
        Args:
            input_video (torch.Tensor): (b, c, t, h, w)。
            start_frame (int).
        ...
        step = 1
        n_frames = input_video.size(2)
        similarity = 1.0  # 初始化相似度为高值
        while step < self.max_step:
            # 确保不越界
            if start_frame + step >= n_frames:
                break
            similarity = self.compute_frame_similarity(
                input_video[:, :, start_frame, :, :],
                input_video[:, :, start_frame + step, :, :]
            )
    ...
    def __call__(self, input_video, reconstruction_video):
        total_loss = 0.0
        t = 0
        n_frames = input_video.size(2)
        i = 0
        ...
        while t < n_frames - 1:
            step = self.compute_dynamic_step(input_video, t)
            # ensure not out of bounds
            next_frame = min(t + step, n_frames - 1)
            
            # Compute loss for current frame
            input_diff = input_video[:, :, next_frame, :, :] - input_video[:, :, t, :, :]
            reconstruction_diff = reconstruction_video[:, :, next_frame, :, :] - reconstruction_video[:, :, t, :, :]
            loss = torch.mean(torch.abs(input_diff - reconstruction_diff))
            
            total_loss += loss
            t += step
            i += 1
        
        #print("Times of long-term loss compute :", i)
        return total_loss/i
