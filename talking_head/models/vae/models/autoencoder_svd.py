...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def init_from_ckpt(
        ...
        if path.endswith('safetensors'): 
            ...
            for k, v in sd.items():
                if "conditioner" in k or "model.diffusion_model." in k:
                    continue 
                        
                new_k = k.replace('first_stage_model.', '')
                new_k = new_k.replace('down_blocks.0.resnets.0.', 'down.0.block.0.')
                ...
                new_k = new_k.replace('.time_mixer.mix_factor', '.mix_factor') 
                new_k = new_k.replace('.spatial_res_block.', '.')  
                ...
                print(f"svd_vae_state_dict, {new_k}")
        else:
            new_sd = sd
    ...
    def get_input(self, batch: Dict) -> torch.Tensor: 
        if batch[self.input_key].ndim == 5:
            print("batch[self.input_key]", batch[self.input_key].shape) 
    ...
    def inner_training_step(
        ...
        additional_decode_kwargs["timesteps"] = batch["timesteps"]
