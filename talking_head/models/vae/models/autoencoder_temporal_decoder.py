...
class AutoencodingEngine(AbstractAutoencoder):
    ...
    def init_from_ckpt(
        ...
        #path = "/mnt/data/yifanzhang/talking-head/logs/2024-07-12T12-27-48_train-sdxl_vae_video_componential_multi/checkpoints/trainstep_checkpoints/epoch=000005.ckpt"
        """
    ...
    def get_input(self, batch: Dict) -> torch.Tensor: 
        ...
        if batch[self.input_key].ndim == 5: 
            num_frames = int(batch[self.input_key].shape[1])
            batch["timesteps"] = int(batch[self.input_key].shape[1])
            x = rearrange(batch[self.input_key], "b t c h w -> (b t) c h w")
            return x, num_frames  
        else:
    ...
    def get_autoencoder_params(self) -> list:
        ...
        if hasattr(self.loss, "get_trainable_autoencoder_parameters"):
            ...
            for p in self.loss.get_trainable_autoencoder_parameters():
                print("Trainable loss param:", p.name)
        ...
        #     # 检查模块名称中是否包含 "time_att"
        #     if "time_att" in name or "causal_3d_conv" in name:
        #         # 如果是，收集该模块的所有参数
        #         #print(f"Found 'time_att' in module: {name}")
    ...
    def forward(
        ...
        #print("additional_decode_kwargs", additional_decode_kwargs)
    ...
    def inner_training_step(
        ...
        with torch.no_grad():
            x, num_frames = self.get_input(batch)
        ...
        if optimizer_idx == 0:
            ...
            self.log(
                "con_loss",
                log_dict_ae["train/loss/consistency_loss"],
                prog_bar=True,
                logger=False,
                on_epoch=False,
                on_step=True,
    ...
    def configure_optimizers(self) -> List[torch.optim.Optimizer]:
        #print("self.trainable_ae_params", self.trainable_ae_params) None
        #print("self.trainable_disc_params", self.trainable_disc_params) None 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            regularizer_config={
                "target": (
                    "talking_head.models.vae.modules.autoencoding.regularizers.quantize" ".VectorQuantizer"
                ),
                "params": {
                    "n_e": n_embed,
                    "e_dim": embed_dim,
