            ...
            if not self.is_image:
                if self.dynamic_frames:
                    if  random.random() < 1:
                        ...
                        num_frames = random.choice([12, 14]) 
                else:
                    num_frames = self.num_frames

                if num_frames>video_length:
                    return self.__getitem__(np.random.choice(self.__len__()))
                clip_length = min(video_length, (num_frames - 1) * self.sample_stride + 1)
                start_idx = random.randint(0, video_length - clip_length)
                batch_idx = np.linspace(start_idx, start_idx + clip_length - 1, num_frames, dtype=int)
            else:
