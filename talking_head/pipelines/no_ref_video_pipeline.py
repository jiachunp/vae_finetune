...
- VideoPipeline: A class that extends the DiffusionPipeline class from the diffusers library to handle face animation tasks.
...
- Create a VideoPipeline instance with the required components.
...
from talking_head.models.mutual_self_attention import torch_dfs
from talking_head.models.ref_attention import TemporalBasicTransformerBlock


@dataclass
class VideoPipelineOutput(BaseOutput):
    """
    VideoPipelineOutput is a custom class that inherits from BaseOutput and represents the output of the VideoPipeline.
    
    Attributes:
        videos (Union[torch.Tensor, np.ndarray]): A tensor or numpy array containing the generated video frames.
    
    Methods:
        __init__(self, videos: Union[torch.Tensor, np.ndarray]): Initializes the VideoPipelineOutput object with the generated video frames.
    """
    videos: Union[torch.Tensor, np.ndarray]

class VideoPipeline(DiffusionPipeline):
    """
    VideoPipeline is a custom DiffusionPipeline for animating faces.
    ...
    Methods:
        __init__(self, vae, denoising_unet, face_locator,
                 image_proj, scheduler): Initializes the VideoPipeline
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ref_image_tensor = rearrange(ref_image, "b f c h w -> (b f) c h w")  # (bs, 1+2, c, h, w)
        ...
        org_pixel_values_full_mask = pixel_values_full_mask
        org_pixel_values_face_mask = pixel_values_face_mask
        org_pixel_values_lip_mask = pixel_values_lip_mask
        pixel_values_full_mask = (
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    ref_image_latents = rearrange(ref_image_latents, "f c h w -> 1 c f h w")
                    ref_sample = torch.cat([ref_image_latents] * 2) if do_classifier_free_guidance else ref_image_latents
                    ref_timestep = torch.zeros_like(t)
                    ref_video_length = ref_image_latents.shape[2]
                    ref_pixel_values_full_mask = (
                        [torch.cat([mask[:ref_video_length].to(device=self.denoising_unet.device, dtype=self.denoising_unet.dtype)] * 2) for mask in org_pixel_values_full_mask]
                        if do_classifier_free_guidance
                        else org_pixel_values_full_mask[:ref_video_length]
                    )
                    ref_pixel_values_face_mask = (
                        [torch.cat([mask[:ref_video_length].to(device=self.denoising_unet.device, dtype=self.denoising_unet.dtype)] * 2) for mask in org_pixel_values_face_mask]
                        if do_classifier_free_guidance
                        else org_pixel_values_face_mask[:ref_video_length]
                    )
                    ref_pixel_values_lip_mask = (
                        [torch.cat([mask[:ref_video_length].to(device=self.denoising_unet.device, dtype=self.denoising_unet.dtype)] * 2) for mask in org_pixel_values_lip_mask]
                        if do_classifier_free_guidance
                        else org_pixel_values_lip_mask[:ref_video_length]
                    )
                    self.denoising_unet(
                        ref_sample,
                        ref_timestep,
                        encoder_hidden_states=encoder_hidden_states,
                        mask_cond_fea=face_mask[:, :, :ref_video_length, :, :],
                        full_mask=ref_pixel_values_full_mask,
                        face_mask=ref_pixel_values_face_mask,
                        lip_mask=ref_pixel_values_lip_mask,
                        audio_embedding=audio_tensor[:, :ref_video_length, :, :],
