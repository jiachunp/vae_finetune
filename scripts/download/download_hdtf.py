...
Usage:
```
$ python download_hdtf.py --output_dir /tmp/data/hdtf --num_workers 8
```
...
def download_video(video_id, download_path, resolution: int=None, video_format="mp4", log_file=None):
    ...
    #     "--no-continue"
    # ]
    video_selection = "bestvideo+bestaudio" if resolution is None else f"bestvideo[height={resolution}]+bestaudio"
    command = [
        "youtube-dl",
        "--format", video_selection,
