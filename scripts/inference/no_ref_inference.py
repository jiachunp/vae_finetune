...
from talking_head.pipelines.no_ref_video_pipeline import VideoPipeline
...
from talking_head.models.ref_unet_3d import UNet3DConditionModel
from talking_head.utils.util import tensor_to_video
...
def inference_process(args: argparse.Namespace):
    ...
    state_dict = torch.load(os.path.join(audio_ckpt_dir, "net.pth"), map_location="cpu")
    new_state_dict = {}
    for key in state_dict.keys():
        if "reference_unet" not in key:
            new_state_dict[key] = state_dict[key]

    # Load the new state dictionary into the model
    m, u = net.load_state_dict(new_state_dict)
